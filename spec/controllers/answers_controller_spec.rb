...
RSpec.describe AnswersController, type: :controller do
  ...
  describe 'POST #comment' do
    ...
    context 'authorized user' do
      ...
      context 'with valid attributes' do
        ...
        context 'by the other user' do
          it 'should be able to add a comment to the question' do
            expect { post :comment, params: { id: answer, comment: { body: comment_attrs[:body] } }, format: :js }
                .to change{ answer.comments.count }.by(1)
          end

          it 'renders comment template' do
            post :comment, params: {  id: answer, comment: { body: comment_attrs[:body] } }, format: :js
      ...
      context 'with invalid attributes' do
        it 'cannot add a comment' do
          expect { post :comment, params: { id: answer, comment: { body: '' } }, format: :js }
    ...
    context 'unauthorized user' do
      it 'cannot add a comment' do
        expect { post :comment, params: { id: answer, comment: { body: 'comment body' } }, format: :js }
            .to_not change{ answer.comments.count }
      end
    end
  end
